...
%    imshow(LesionData,[]);
    ...
    temp_mean = mean(histoData); %Media
    temp_m2 = moment(histoData,2);
    temp_m3 = moment(histoData,3);
    temp_m4 = moment(histoData,4);
    
    masked_m2 = sqrt(temp_m2); %Std deviation
    masked_m3 = power(abs(temp_m3),1/3).*sign(temp_m3); %Kurtosis
    masked_m4 = power(temp_m4,1/4); %Skewness
